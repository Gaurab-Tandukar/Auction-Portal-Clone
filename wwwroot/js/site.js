...
(function () {
    ...
    function formatDate(dateStr) {
        ...
        return 'Ends ' + d.toLocaleDateString('en-NP', { day: 'numeric', month: 'short', year: 'numeric' });
    }
    ...
    function renderItems(items) {
        ...
        items.forEach(function (item, idx) {
            ...
            const thumbHtml = item.thumbnailUrl
                ? `<img src="${escapeHtml(item.thumbnailUrl)}" alt="${escapeHtml(item.title)}" class="wishlist-item-thumb">`
    ...
    function removeItem(auctionItemId, cardEl) {
        ...
        .then(function () {
            ...
            const remaining = contentEl.querySelectorAll('.wishlist-item-card').length;
            updateBadge(remaining);
            if (remaining === 0) {
                setTimeout(function () { renderItems([]); }, 280);
            }
        })
        .catch(function () {
